...
module.exports = {
  landingPage: async (req, res) => {
    try {
      ...
      res.status(200).json({ data: vocher });
    } catch (err) {
  ...
  detailPage: async (req, res) => {
    try {
      ...
      if (!vocher) {
        return res
          .status(404)
          .json({ message: "vocher game tidak ditemukan.!" });
      }
  ...
  checkout: async (req, res) => {
    try {
      const { accountUser, name, nominal, vocher, payment, bank } = req.body;

      const res_vocher = await Vocher.findOne({ _id: vocher })
      ...
      const payload = {
        historyVocherTopup: {
          gameName: res_vocher._doc.name,
          category: res_vocher._doc.category
            ? res_vocher._doc.category.name
            : "",
          thumbnail: res_vocher._doc.thumbnail,
        ...
        historyUser: {
          name: res_vocher._doc.user?.name,
          phoneNumber: res_vocher._doc.user?.phoneNumber,
        },

        category: res_vocher._doc.category?._id,
        user: res_vocher._doc.user?._id,
      };
